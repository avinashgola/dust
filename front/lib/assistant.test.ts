...
describe("isModelAvailable", () => {
  ...
  it("should return false when the model's unavailableIfFeatureFlag is enabled, even if another condition grants access", () => {
    const model = createMockModel({
      availableIfOneOf: { featureFlag: "deepseek_feature" },
      unavailableIfFeatureFlag: "disable_gpt_6_astra",
      largeModel: false,
    });
    const plan = createMockPlan(PRO_PLAN_SEAT_29_CODE);
  ...
  it("should return true when the model's unavailableIfFeatureFlag is not enabled", () => {
    const model = createMockModel({
      unavailableIfFeatureFlag: "disable_gpt_6_astra",
